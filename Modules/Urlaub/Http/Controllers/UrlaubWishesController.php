...
/**
 * Class UrlaubWishesController.
 */
class UrlaubWishesController extends Controller
{
    ...
    public function validateTokenList($token)
    {
        ...
        if ($this->wish->validateToken($token)) {
            if (Route::has('urlaub.list')) {
                return redirect()->route('urlaub.list');
            }
        }
    ...
    public function wishList(ManageWishesRequest $request)
    {
        ...
        return view('urlaub::wish.index')->with([
            'status'     => $this->status,
