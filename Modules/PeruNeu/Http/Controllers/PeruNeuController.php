...
namespace Modules\PeruNeu\Http\Controllers;
...
use Modules\PeruNeu\Http\Requests\StoreWishRequest;

class PeruNeuController extends Controller
{
    protected $adults = [];
    protected $kids = [];
    protected $catering = [];
    ...
    public function __construct(WhitelabelsRepository $whitelabel, CategoriesRepository $categories, EloquentAttachmentsRepository $attachements)
    {
        ...
        $this->whitelabelId = \Config::get('peruneu.id');
    }
    ...
    {
        $whitelabel = $this->whitelabel->getByName('PeruNeu');

        return view('peruneu::index')->with([
    ...
    public function store(StoreWishRequest $request, UserRepository $user, WishesRepository $wish)
    {
        ...
        $html = view('peruneu::layer.created')->with([
