...
namespace Modules\Wale\Http\Controllers;
...
use Modules\Wale\Http\Requests\StoreWishRequest;

class WaleController extends Controller
{
    protected $adults = [];
    protected $kids = [];
    protected $catering = [];
    ...
    public function __construct(WhitelabelsRepository $whitelabel, CategoriesRepository $categories, EloquentAttachmentsRepository $attachements)
    {
        ...
        $this->whitelabelId = \Config::get('wale.id');
    }
    ...
    {
        $whitelabel = $this->whitelabel->getByName('Wale');

        return view('wale::index')->with([
    ...
    public function store(StoreWishRequest $request, UserRepository $user, WishesRepository $wish)
    {
        ...
        $html = view('wale::layer.created')->with([
