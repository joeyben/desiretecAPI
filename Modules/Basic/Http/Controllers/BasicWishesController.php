...
/**
 * Class BasicWishesController.
 */
class BasicWishesController extends Controller
{
    ...
    public function validateTokenList($token)
    {
        ...
        if ($this->wish->validateToken($token)) {
            if (Route::has('basic.list')) {
                return redirect()->route('basic.list');
            }
        }
    ...
    public function wishList(ManageWishesRequest $request)
    {
        ...
        return view('basic::wish.index')->with([
            'status'     => $this->status,
