...
import CssConfig from './CssConfig.js'
  ...
  Vue.component('custom-user-letter', config.customUserLetter)
  Vue.component('custom-link-by-from', config.customLinkByFrom)
  ...
  export default {
    ...
    data () {
      return {
        apiUrl: window.laroute.route('inbox.view'),
    ...
    methods: {
      ...
      formatDate (value, fmt = 'D MMM YYYY') {
        return (value == null)
          ? ''
          : moment(value, moment.ISO_8601).format(fmt)
      },
      fromNow (value) {
        return (value == null)
          ? ''
      ...
      onDelete (id) {
        this.$store.dispatch('block', {element: 'notificationsComponent', load: true})
        this.$http.delete(window.laroute.route('notifications.destroy', {id: id}))
      ...
      onForceDelete (id) {
        ...
        this.$http.delete(laroute.route('notifications.forceDelete', {id: id}))
          .then(this.onDeleteSuccess)
          .catch(this.onFailed)
          .then(() => {
            this.$store.dispatch('block', {element: 'notificationsComponent', load: false})
      ...
      onRestore (id) {
        this.$store.dispatch('block', {element: 'notificationsComponent', load: true})
        // eslint-disable-next-line
        this.$http.put(window.laroute.route('notifications.restore', {id: id}))
          .then(this.onDeleteSuccess)
          .catch(this.onFailed)
          .then(() => {
            this.$store.dispatch('block', {element: 'notificationsComponent', load: false})
