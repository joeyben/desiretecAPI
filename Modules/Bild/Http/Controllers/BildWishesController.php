...
/**
 * Class BildWishesController.
 */
class BildWishesController extends Controller
{
    ...
    public function validateTokenList($token)
    {
        if ($this->wish->validateToken($token)) {
            if (Route::has('bild.list')) {
                return redirect()->route('bild.list');
            }
        }
    ...
    public function wishList(ManageWishesRequest $request)
    {
        ...
        return view('bild::wish.index')->with([
            'status'     => $this->status,
