...
class AutooffersNovasolController extends Controller
{
    ...
    public function show(Wish $wish)
    {
        ...
        foreach ($autooffers as $autooffer) {
            ...
            $types[] = DB::table('offer_files')->select('type')->where('offer_id', $autooffer->code)->get();
        }
    ...
    public function toTheOffer($offer_id)
    {
        ...
        if (null !== $autooffer) {
            ...
            $url .= str_replace(' ', '-', mb_strtolower($novasol_country->name)) . '/';
            $url .= str_replace(' ', '-', mb_strtolower($novasol_area->name)) . '/';
            $url .= str_replace(' ', '-', 'ferienwohnung-' . mb_strtolower($autooffer->hotel_location_name)) . '-' . mb_strtolower($autooffer->hotel_code) . '?';
            $url .= 'agency=2051402&';
            ...
            foreach ($product->pictures as $picture) {
                foreach ($picture->picture as $pic) {
                    $arr[] = [
                        ...
                        'type'     => $product->features->feature[0]->subgroup
                    ];
                }
            }
