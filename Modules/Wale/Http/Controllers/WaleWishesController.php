...
/**
 * Class WaleWishesController.
 */
class WaleWishesController extends Controller
{
    ...
    public function view(Wish $wish)
    {
        ...
        return view('wale::wish.wish')->with([
            'wish'               => $wish,
            'avatar'             => $avatar,
    ...
    public function validateTokenList($token)
    {
        ...
        if ($this->wish->validateToken($token)) {
            if (Route::has('wale.list')) {
                return redirect()->route('wale.list');
            }
        }
    ...
    {
        //var_dump($request->ip());
        return view('wale::wish.index')->with([
            'status'     => $this->status,
