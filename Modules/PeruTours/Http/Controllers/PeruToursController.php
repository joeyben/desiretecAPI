...
namespace Modules\PeruTours\Http\Controllers;
...
use Modules\PeruTours\Http\Requests\StoreWishRequest;

class PeruToursController extends Controller
{
    protected $adults = [];
    protected $kids = [];
    protected $catering = [];
    ...
    public function __construct(WhitelabelsRepository $whitelabel, CategoriesRepository $categories, EloquentAttachmentsRepository $attachements)
    {
        ...
        $this->whitelabelId = \Config::get('perutours.id');
    }
    ...
    {
        $whitelabel = $this->whitelabel->getByName('PeruTours');

        return view('perutours::index')->with([
    ...
    {
        $html = view('perutours::layer.popup')->with([
    ...
    public function store(StoreWishRequest $request, UserRepository $user, WishesRepository $wish)
    {
        ...
        $html = view('perutours::layer.created')->with([
