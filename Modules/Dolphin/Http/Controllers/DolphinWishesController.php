...
namespace Modules\Dolphin\Http\Controllers;
...
/**
 * Class DolphinWishesController.
 */
class DolphinWishesController extends Controller
{
    ...
    public function __construct(WishesRepository $wish, WhitelabelsRepository $whitelabel, CategoriesRepository $categories)
    {
        ...
        $this->whitelabelId = \Config::get('dolphin.id');
    ...
    public function view(Wish $wish)
    {
        ...
        return view('dolphin::wish.wish')->with([
            'wish'               => $wish,
            'avatar'             => $avatar,
    ...
    public function validateTokenList($token)
    {
        ...
        if ($this->wish->validateToken($token)) {
            if (Route::has('dolphin.list')) {
                return redirect()->route('dolphin.list');
            }
        }
    ...
    {
        //var_dump($request->ip());
        return view('dolphin::wish.index')->with([
            'status'     => $this->status,
