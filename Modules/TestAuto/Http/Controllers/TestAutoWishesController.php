...
namespace Modules\TestAuto\Http\Controllers;
...
/**
 * Class TestAutoWishesController.
 */
class TestAutoWishesController extends Controller
{
    ...
    public function __construct(WishesRepository $wish, WhitelabelsRepository $whitelabel, CategoriesRepository $categories)
    {
        ...
        $this->whitelabelId = \Config::get('testauto.id');
    ...
    public function view(Wish $wish)
    {
        ...
        return view('testauto::wish.wish')->with([
            'wish'               => $wish,
            'avatar'             => $avatar,
    ...
    public function validateTokenList($token)
    {
        ...
        if ($this->wish->validateToken($token)) {
            if (Route::has('testauto.list')) {
                return redirect()->route('testauto.list');
            }
        }
    ...
    {
        //var_dump($request->ip());
        return view('testauto::wish.index')->with([
            'status'     => $this->status,
