...
namespace Modules\Demoatw\Http\Controllers;
...
use Modules\Demoatw\Http\Requests\StoreWishRequest;

class DemoatwController extends Controller
{
    protected $adults = [];
    protected $kids = [];
    protected $catering = [];
    ...
    public function __construct(WhitelabelsRepository $whitelabel, CategoriesRepository $categories, EloquentAttachmentsRepository $attachements)
    {
        ...
        $this->whitelabelId = \Config::get('demoatw.id');
    }
    ...
    {
        $whitelabel = $this->whitelabel->getByName('Demoatw');

        return view('demoatw::index')->with([
    ...
    public function store(StoreWishRequest $request, UserRepository $user, WishesRepository $wish)
    {
        ...
        $html = view('demoatw::layer.created')->with([
