...
/**
 * Class DemoatwWishesController.
 */
class DemoatwWishesController extends Controller
{
    ...
    public function view(Wish $wish)
    {
        ...
        return view('demoatw::wish.wish')->with([
            'wish'               => $wish,
            'avatar'             => $avatar,
    ...
    public function validateTokenList($token)
    {
        ...
        if ($this->wish->validateToken($token)) {
            if (Route::has('demoatw.list')) {
                return redirect()->route('demoatw.list');
            }
        }
