...
namespace Modules\FN\Http\Controllers;
...
/**
 * Class FNWishesController.
 */
class FNWishesController extends Controller
{
    ...
    public function __construct(WishesRepository $wish, WhitelabelsRepository $whitelabel, CategoriesRepository $categories)
    {
        ...
        $this->whitelabelId = \Config::get('fn.id');
    ...
    public function validateTokenList($token)
    {
        if ($this->wish->validateToken($token)) {
            if (Route::has('fn.list')) {
                return redirect()->route('fn.list');
            }
        }
    ...
    {
        //var_dump($request->ip());
        return view('fn::wish.index')->with([
            'status'     => $this->status,
