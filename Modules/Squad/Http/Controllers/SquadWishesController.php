...
namespace Modules\Squad\Http\Controllers;
...
/**
 * Class SquadWishesController.
 */
class SquadWishesController extends Controller
{
    ...
    public function __construct(WishesRepository $wish, WhitelabelsRepository $whitelabel, CategoriesRepository $categories)
    {
        ...
        $this->whitelabelId = \Config::get('squad.id');
    ...
    public function view(Wish $wish)
    {
        ...
        return view('squad::wish.wish')->with([
            'wish'               => $wish,
            'avatar'             => $avatar,
    ...
    public function validateTokenList($token)
    {
        ...
        if ($this->wish->validateToken($token)) {
            if (Route::has('squad.list')) {
                return redirect()->route('squad.list');
            }
        }
    ...
    {
        //var_dump($request->ip());
        return view('squad::wish.index')->with([
            'status'     => $this->status,
