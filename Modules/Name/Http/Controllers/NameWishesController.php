...
/**
 * Class NameWishesController.
 */
class NameWishesController extends Controller
{
    ...
    public function __construct(WishesRepository $wish, WhitelabelsRepository $whitelabel, CategoriesRepository $categories)
    {
        ...
        $this->whitelabelId = \Config::get('name.id');
    ...
    public function view(Wish $wish)
    {
        ...
        return view('name::wish.wish')->with([
            'wish'               => $wish,
            'avatar'             => $avatar,
    ...
    public function validateTokenList($token)
    {
        ...
        if ($this->wish->validateToken($token)) {
            if (Route::has('name.list')) {
                return redirect()->route('name.list');
            }
        }
    ...
    {
        //var_dump($request->ip());
        return view('name::wish.index')->with([
            'status'     => $this->status,
