...
(function ($) {
    var Debug = {
        ...
        log: function log(message, color) {
            if (!this.enabled) {
                return;
            }
    ...
    kwizzme.AbstractTripDataDecoder = {
        ...
        matchesUrl: '',
        trim: function trim(str, pattern) {
        ...
        formArrayToObject: function formArrayToObject(arr) {
            var obj = {};
            console.log(arr);
        ...
        decodeFilterData: function decodeFilterData(form, formData) {
            var tripData = {};
        ...
        getSelectText: function getSelectText(name, value, trimPattern) {
            return this.trim(jQuery(this.filterFormSelector).find('select[name="' + name + '"] option[value="' + value + '"]').text(), trimPattern);
        },
        dictionaryTransformValue: function dictionaryTransformValue(dictionary, key, prop) {
        ...
        dictionaryTransformArray: function dictionaryTransformArray(dictionary, keys, prop) {
        ...
        getTrackingLabel: function getTrackingLabel(tripData, variant) {
            return variant;
        }
    };
    ...
    kwizzme.PopupManager = {
        ...
        shown: false,
        isFromPaidTraffic: function isFromPaidTraffic() {
        ...
        setBack: function setBack(event) {
            jQuery('input[name="step"]').val(1);
            this.back = true;
            this.onFormSubmit(event);
        },
        setNext: function setNext(event) {
            this.next = true;
            this.onFormSubmit(event);
        },
        ...
        installStyles: function installStyles() {
            jQuery('head').append(jQuery('<link rel="stylesheet" type="text/css" media="all" href="' + this.config.baseUrl + this.config.cssPath + '"/>'));
        },
        initExitIntent: function initExitIntent() {
        ...
        //    }
        //},
        selectDecoder: function selectDecoder() {
        ...
        fetchPopup: function fetchPopup() {
            var tripData;
        ...
        onPopupFetched: function onPopupFetched(data, status, jqxhr) {
        ...
        changeTexts: function changeTexts(headline, tagline) {
        ...
        showContent: function showContent(content) {
            ...
            if ($(".kwp-content").hasClass('kwp-completed-tui') || $(".kwp-content").hasClass('kwp-completed')) {
                ...
                if (kwizzme.PopupManager.decoder.name == "TUI IBE") {} else {}
            }
        },
        ...
        createPopup: function createPopup() {
            if (null === this.popup) {
                ...
                this.modal = jQuery('<div/>', { 'class': 'kwizzme-modal' }).hide().append(this.popup)
        ...
        getQueryPart: function getQueryPart() {
            var part = '';
    ...
    kwizzme.Tracking = {
        ...
        init: function init(category, TrackingId) {
        ...
        initMixpanel: function initMixpanel(siteId) {
        ...
        rawEvent: function rawEvent(category, action, label) {
            ga('DesireTec.send', 'event', category, action, label);
        },
        event: function event(action, label) {
...
(function ($) {
    ...
    kwizzme.popupTemplate = function (variant) {
        ...
        var mobileHeader = kwizzme.PopupManager.decoder.getRandomElement(['Jetzt Ihre Reise wünschen und Angebot erhalten!', 'Dürfen wir Sie beraten?', 'Hier klicken und persönliches Angebot erhalten', 'Nicht das Passende gefunden?']);
        ...
        return '' + '<div class="kwp-header kwp-variant-' + variant + '">' + '<div class="kwp-close-button kwp-close"></div>' + '<div class="kwp-overlay"></div>' + '<div class="kwp-header-content">' + '<h1>' + texts[variant].header + ' <br/>' + '</h1>' + '<p>' + texts[variant].body + '</p>' + '</div>' + '</div>' + '<div class="kwp-body ' + variant + '-body">' + '</div><div style="clear:both;"></div>';
    };
    ...
    var TuiIBETripDataDecoderMobile = $.extend({}, kwizzme.AbstractTripDataDecoder, {
        ...
        filterDataDecoders: {
            'catering': function catering(form, formData) {
                return '';
            },
            'hotel_category': function hotel_category(form, formData) {
                return '';
            },
            'destination': function destination(form, formData) {
                return $(".search-criteria-list li:eq( 0 ) strong").attr('title');
            },
            'pax': function pax(form, formData) {
                var pax = $(".search-criteria-list li:eq( 4 ) strong").attr('title').split(',');
                return parseInt(pax[0].replace(' Erwachsene', ''));
            },
            'budget': function budget(form, formData) {
                return '';
            },
            'children': function children(form, formData) {
                var pax = $(".search-criteria-list li:eq( 4 ) strong").attr('title').split(',');
                return parseInt(pax[1].replace(' Kinder', ''));
            },
            'age_1': function age_1(form, formData) {
                return '';
            },
            'age_2': function age_2(form, formData) {
                return '';
            },
            'age_3': function age_3(form, formData) {
                return '';
            },
            'earliest_start': function earliest_start(form, formData) {
                var latest_return = $(".search-criteria-list li:eq( 3 )").text().split(' - ');
                return $.trim(latest_return[0]);
            },
            'latest_return': function latest_return(form, formData) {
                var latest_return = $(".search-criteria-list li:eq( 3 )").text().split(' - ');
                return $.trim(latest_return[1]);
            },
            'duration': function duration(form, formData) {
            ...
            'extra': function extra(form, formData) {
                return '';
            },
            'room_type': function room_type(form, formData) {
                return '';
            },
            'cities': function cities(form, formData) {
                return '';
            },
            'airport': function airport(form, formData) {
                return $(".search-criteria-list li:eq( 1 ) strong").attr('title');
            },
            'is_popup_allowed': function is_popup_allowed(form, formData) {
    ...
    var TuiIBETripDataDecoder = $.extend({}, kwizzme.AbstractTripDataDecoder, {
        decodeDate: function decodeDate(raw) {
        ...
        getAirportName: function getAirportName(code) {
            if (!this.airports) {
                this.airports = {};

                var data = this.getScope().IbeApi.state.referenceData.airports.slice();

                data = data.concat([{ code: 'WEST', name: 'Deutschland West' }, { code: 'EAST', name: 'Deutschland Ost' }, { code: 'NORTH', name: 'Deutschland Nord' }, { code: 'SOUTH', name: 'Deutschland Süd' }]);
        ...
        getCountryIdFromRegionId: function getCountryIdFromRegionId(code) {
            if (!this.regions) {
                this.regions = {};
        ...
        getVariant: function getVariant() {
            ...
            if (regex.test(String(window.location))) {
                ...
                return this.getRandomElement(['eil-n1', 'eil-n1', 'eil-n2', 'eil-n5']);
            }
        },
        getTrackingLabel: function getTrackingLabel(tripData, variant) {
            var destId = null,
    ...
    var KwizzmeFakeTripDataDecoder = $.extend({}, kwizzme.AbstractTripDataDecoder, {
        ...
        getVariant: function getVariant() {
            if (isMobile()) {
                ...
                return this.getRandomElement(['eil-n1-social']);
            } else {
                return this.getRandomElement(['eil-n1', 'eil-n1', 'eil-n2', 'eil-n5']);
            }
        }
    ...
    kwizzme.scrollUpDetect = function () {
        var shown = false;
        $('body').swipe({ swipeStatus: function swipeStatus(event, phase, direction, distance) {
        ...
        $(".kwizzme-modal").swipe({
            ...
            swipe: function swipe(e, direction, distance, duration, fingerCount, fingerData) {
                var $event = e;
                if (direction === "left") {
                    if ($(this).hasClass('m-open')) return false;
    ...
    function textareaAutosize() {
        $(document).one('focus.textarea', '.kwp textarea', function () {
