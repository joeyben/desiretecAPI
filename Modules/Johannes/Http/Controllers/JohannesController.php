...
namespace Modules\Johannes\Http\Controllers;
...
use Modules\Johannes\Http\Requests\StoreWishRequest;

class JohannesController extends Controller
{
    protected $adults = [];
    protected $kids = [];
    protected $catering = [];
    ...
    public function __construct(WhitelabelsRepository $whitelabel, CategoriesRepository $categories, EloquentAttachmentsRepository $attachements)
    {
        ...
        $this->whitelabelId = \Config::get('johannes.id');
    }
    ...
    {
        $whitelabel = $this->whitelabel->getByName('Johannes');

        return view('johannes::index')->with([
    ...
    public function store(StoreWishRequest $request, UserRepository $user, WishesRepository $wish)
    {
        ...
        $html = view('johannes::layer.created')->with([
