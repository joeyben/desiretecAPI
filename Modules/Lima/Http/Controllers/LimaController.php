...
use Modules\Lima\Http\Requests\StoreWishRequest;
...
class LimaController extends Controller
{
    ...
    public function __construct(WhitelabelsRepository $whitelabel, CategoriesRepository $categories, EloquentAttachmentsRepository $attachements)
    {
        ...
        $this->whitelabelId = \Config::get('Lima.id');
    }
    ...
    {
        $whitelabel = $this->whitelabel->getByName('Lima');

        return view('lima::index')->with([
    ...
    {
        $html = view('lima::layer.popup')->with([
    ...
    public function store(StoreWishRequest $request, UserRepository $user, WishesRepository $wish)
    {
        ...
        $html = view('lima::layer.created')->with([
