...
namespace Modules\Adventure\Http\Controllers;
...
/**
 * Class AdventureWishesController.
 */
class AdventureWishesController extends Controller
{
    ...
    public function __construct(WishesRepository $wish, WhitelabelsRepository $whitelabel, CategoriesRepository $categories)
    {
        ...
        $this->whitelabelId = \Config::get('adventure.id');
    ...
    public function view(Wish $wish)
    {
        ...
        return view('adventure::wish.wish')->with([
            'wish'               => $wish,
            'avatar'             => $avatar,
    ...
    public function validateTokenList($token)
    {
        ...
        if ($this->wish->validateToken($token)) {
            if (Route::has('adventure.list')) {
                return redirect()->route('adventure.list');
            }
        }
    ...
    {
        //var_dump($request->ip());
        return view('adventure::wish.index')->with([
            'status'     => $this->status,
