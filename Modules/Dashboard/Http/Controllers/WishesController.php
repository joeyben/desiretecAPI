...
class WishesController extends Controller
{
    ...
    public function byDay(Request $request)
    {
        try {
            $data = $this->wishes->withCriteria([
                new WhereBetween('wishes.created_at', $this->carbon->startOfMonth()->toDateString(), $this->carbon->endOfMonth()->toDateString()),
                new ByWhitelabel(),
                new Where('wishes.whitelabel_id', $request->get('whitelabelId')),
                new GroupBy('day')
            ...
            foreach ($data as $item) {
                $items[$item->day] = [$item->created_at->timestamp, $item->wishes_count];
            }
