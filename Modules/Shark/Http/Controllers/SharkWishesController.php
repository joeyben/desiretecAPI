...
namespace Modules\Shark\Http\Controllers;
...
/**
 * Class SharkWishesController.
 */
class SharkWishesController extends Controller
{
    ...
    public function __construct(WishesRepository $wish, WhitelabelsRepository $whitelabel, CategoriesRepository $categories)
    {
        ...
        $this->whitelabelId = \Config::get('shark.id');
    ...
    public function view(Wish $wish)
    {
        ...
        return view('shark::wish.wish')->with([
            'wish'               => $wish,
            'avatar'             => $avatar,
    ...
    public function validateTokenList($token)
    {
        ...
        if ($this->wish->validateToken($token)) {
            if (Route::has('shark.list')) {
                return redirect()->route('shark.list');
            }
        }
    ...
    {
        //var_dump($request->ip());
        return view('shark::wish.index')->with([
            'status'     => $this->status,
