...
/**
 * Class UrlaubsentenWishesController.
 */
class UrlaubsentenWishesController extends Controller
{
    ...
    public function validateTokenList($token)
    {
        ...
        if ($this->wish->validateToken($token)) {
            if (Route::has('urlaubsenten.list')) {
                return redirect()->route('urlaubsenten.list');
            }
        }
    ...
    public function wishList(ManageWishesRequest $request)
    {
        ...
        return view('urlaubsenten::wish.index')->with([
            'status'     => $this->status,
