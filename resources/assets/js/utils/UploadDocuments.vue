<template>
    <div class="table-responsive">
        <el-upload
                ...
                :accept="accept"
                :data="data">
            <el-button size="small" type="primary">{{ trans('modals.add_document') }}</el-button>
  ...
  export default {
    ...
    data () {
      return {
        ...
        errors: new Errors(),
        url: window.laroute.route('documents.store'),
        deleteUrl: 'documents.destroy',
        dialogImageUrl: '',
        dialogVisible: false,
        headers: {
    ...
    methods: {
      ...
      handleRemove (file, fileList) {
        if (file.hasOwnProperty('response')) {
          this.onDestroy(file.response.document.id)
        } else {
      ...
      handleSuccess (response, file, fileList) {
        this.$events.fire('handle-success-document', response)
      },
        ...
        this.$http.delete(window.laroute.route(this.deleteUrl, {id: id}))
          .then((response) => {
            if (response.data.hasOwnProperty('success') && response.data.success === true) {
              this.$events.fire('handle-remove-document', response)
            } else {
              this.$message({
