...
namespace Modules\Ben\Http\Controllers;
...
/**
 * Class BenWishesController.
 */
class BenWishesController extends Controller
{
    ...
    public function __construct(WishesRepository $wish, WhitelabelsRepository $whitelabel, CategoriesRepository $categories)
    {
        ...
        $this->whitelabelId = \Config::get('ben.id');
    ...
    public function view(Wish $wish)
    {
        ...
        return view('ben::wish.wish')->with([
            'wish'                => $wish,
            'avatar'              => $avatar,
    ...
    public function validateTokenList($token)
    {
        if ($this->wish->validateToken($token)) {
            if (Route::has('ben.list')) {
                return redirect()->route('ben.list');
            }
        }
    ...
    {
        //var_dump($request->ip());
        return view('ben::wish.index')->with([
            'status'     => $this->status,
