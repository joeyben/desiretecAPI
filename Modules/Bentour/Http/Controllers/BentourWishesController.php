...
/**
 * Class BentourWishesController.
 */
class BentourWishesController extends Controller
{
    ...
    public function validateTokenList($token)
    {
        ...
        if ($this->wish->validateToken($token)) {
            if (Route::has('bentour.list')) {
                return redirect()->route('bentour.list');
            }
        }
    ...
    public function wishList(ManageWishesRequest $request)
    {
        ...
        return view('bentour::wish.index')->with([
            'status'     => $this->status,
